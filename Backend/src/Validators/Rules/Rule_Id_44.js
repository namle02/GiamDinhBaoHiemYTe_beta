...
const validateRule_Id_44 = async (patientData) => {
    ...
    try {
        ...
        dsDichVu.forEach((item, idx) => {
            ...
            // Nếu (số lượng * 24) > số giờ chênh lệch thì CHẶN; ngược lại thì hợp lệ
            if (soLuongHours > diffHours) {
                result.isValid = false;
                result.errors.push({
                    Id: item.id || item.Id,
                    Error: `Chặn thanh toán BHYT: Số lượng quy đổi (${soLuongHours} giờ = ${soLuong}*24) vượt quá số giờ chênh lệch (${diffHours} giờ) giữa ngày kết quả và ngày thực hiện y lệnh`
                });
            }
        });
