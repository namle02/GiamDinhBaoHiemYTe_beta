using CommunityToolkit.Mvvm.ComponentModel;
using WPF_GiamDinhBaoHiem.Services.Interface;

namespace WPF_GiamDinhBaoHiem.Repos.Model
{
    public partial class XML3 : ObservableObject, IHasStt
    {
        [ObservableProperty] private int id;
        [ObservableProperty] private string? ma_Lk;
        [ObservableProperty] private int? stt;
        [ObservableProperty] private string? ma_Dich_Vu;
        [ObservableProperty] private string? ma_Pttt_Qt;
        [ObservableProperty] private string? ma_Vat_Tu;
        [ObservableProperty] private int? ma_Nhom;
        [ObservableProperty] private string? goi_Vtyt;
        [ObservableProperty] private string? ten_Vat_Tu;
        [ObservableProperty] private string? ten_Dich_Vu;
        [ObservableProperty] private string? ma_Xang_Dau;
        [ObservableProperty] private string? don_Vi_Tinh;
        [ObservableProperty] private int? pham_Vi;
        [ObservableProperty] private decimal? so_Luong;
        [ObservableProperty] private decimal? don_Gia_Bv;
        [ObservableProperty] private decimal? don_Gia_Bh;
        [ObservableProperty] private string? tt_Thau;
        [ObservableProperty] private decimal? tyle_Tt_Dv;
        [ObservableProperty] private decimal? tyle_Tt_Bh;
        [ObservableProperty] private decimal? thanh_Tien_Bv;
        [ObservableProperty] private decimal? thanh_Tien_Bh;
        [ObservableProperty] private decimal? t_TranTt;
        [ObservableProperty] private decimal? muc_Huong;
        [ObservableProperty] private decimal? t_NguonKhac_Nsnn;
        [ObservableProperty] private decimal? t_NguonKhac_Vtnn;
        [ObservableProperty] private decimal? t_NguonKhac_Vttn;
        [ObservableProperty] private decimal? t_NguonKhac_Cl;
        [ObservableProperty] private decimal? t_NguonKhac;
        [ObservableProperty] private decimal? t_Bntt;
        [ObservableProperty] private decimal? t_Bncct;
        [ObservableProperty] private decimal? t_Bhtt;
        [ObservableProperty] private string? ma_Khoa;
        [ObservableProperty] private string? ma_Giuong;
        [ObservableProperty] private string? ma_Bac_Si;
        [ObservableProperty] private string? nguoi_Thuc_Hien;
        [ObservableProperty] private string? ma_Benh;
        [ObservableProperty] private string? ma_Benh_Yhct;
        [ObservableProperty] private string? ngay_Yl;
        [ObservableProperty] private string? ngay_Th_Yl;
        private bool _isError;
        public bool IsError 
        { 
            get => _isError;
            set 
            {
                _isError = value;
                OnPropertyChanged();
            }
        }
        [ObservableProperty] private string? ngay_Kq;
        [ObservableProperty] private int? ma_Pttt;
        [ObservableProperty] private int? vet_Thuong_Tp;
        [ObservableProperty] private int? pp_Vo_Cam;
        [ObservableProperty] private int? vi_Tri_Th_Dvkt;
        [ObservableProperty] private string? ma_May;
        [ObservableProperty] private string? ma_Hieu_Sp;
        [ObservableProperty] private string? tai_Su_Dung;
        [ObservableProperty] private string? du_Phong;
        [ObservableProperty] private string? loaiBenhPham_Id;
        [ObservableProperty] private int? chucdanh_id;
        [ObservableProperty] private string? ketQua;
        [ObservableProperty] private string? mucBinhThuong;
        [ObservableProperty] private string? mo_Ta_Text;
        [ObservableProperty] private string? trinhTuThucHien;



    }
}
