...
-- GIỮ NGUYÊN logic xncpct UNION ALL, bỏ select * 
	SELECT
		...
		-- BỎ: LEFT JOIN DM_PhongBan pb (không dùng trong SELECT)
		...
		left join DM_DIchVU con2 (nolock) on con2.CapTren_Id = con.DichVu_ID
		...
		left join vw_NhanVien bsi (nolock) on bsi.NhanVien_Id = kq.BacSiKetLuan_Id
		-- BỎ: LEFT JOIN Lst_Dictionary mm (không dùng trong SELECT)
