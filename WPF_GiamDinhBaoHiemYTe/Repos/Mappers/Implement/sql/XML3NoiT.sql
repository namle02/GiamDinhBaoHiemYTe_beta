-- =============================================
-- TT03-NoiTru.sql - PHIÊN BẢN TỐI ƯU
-- Bảng kê 03 chi tiết dịch vụ kỹ thuật, VTYT - Giám định BHYT nội trú
...
-- ========== PHẦN 3: Lấy thông tin BenhAn + ICD (bỏ SELECT *) ==========
...
-- ========== PHẦN 4: AppSettings + UDFs ==========
	...
	from Sys_AppSettings (nolock) where Code = N'MaBenhVien_BHYT'

	Declare @MocThoiGian_DuocTyLe varchar(10) = '20190101'
	Declare @MinBHYTChiTra decimal(18,2) = 208500.00
	SELECT @MinBHYTChiTra = VALUE from sys_appsettings (nolock) where code = 'LuongToiThieu'
...
-- ========== PHẦN 6: PTTT + Số chuyển viện (bỏ subquery SELECT *) ==========
...
SELECT		
			@SoChuyenVien = right (BenhAnTongQuat.SoBenhAn,9)
	FROM dbo.BenhAn ba (nolock)
	JOIN BenhAnTongQuat (nolock) ON BenhAnTongQuat.BenhAn_Id = ba.BenhAn_Id
	JOIN BenhAnTongQuat_GCV (nolock) ON BenhAnTongQuat.BenhAnTongQuat_Id = BenhAnTongQuat_GCV.BenhAnTongQuat_Id
	WHERE ba.BenhAn_Id = @BenhAn_Id AND ba.XacNhanChiPhi_Id IS NOT NULL

-- ========== PHẦN 8: Tính trước NoiTru_TraThuocChiTiet → #TraThuoc ==========
DROP TABLE IF EXISTS #TraThuoc;
SELECT 
	ToaThuoc_Id,
	SoLuongTra = SUM(SoLuong)
INTO #TraThuoc
FROM NoiTru_TraThuocChiTiet (nolock)
GROUP BY ToaThuoc_Id

-- ========== PHẦN 9: Truy vấn chính (CTE cho computed columns) ==========
;WITH CTE_Base AS (
	SELECT
			Ma_Lk = @Ma_Lk
			...
			, ma_nhom = CASE
						...
						WHEN LI.PhanNhom = 'DU' AND (d.BHYT = 1 or (xncpct.DonGiaHoTroChiTra>0)) and ld.LoaiVatTu_Id = ('V') and ld.MaLoaiDuoc not in ('LD0143','VTYT003') And (ISNULL(dtl.TyLe,0) > 0 OR TT04.VATTU_TT04_ID IS NOT NULL) OR  map.TenField in ('10','VTYT')  THEN '10'
			...
			-- ===== COMPUTED: adj_qty (số lượng đã điều chỉnh PT50/PT80/Ghep) =====
			, adj_qty = CAST(CASE 
				WHEN clsyc.PT50 = 1 AND (map.TenField = '12' Or map.TenField = '07') THEN 0.5
				WHEN clsyc.PT80 = 1 And ( map.TenField = '07') THEN 0.8
				ELSE CAST((sum(xncpct.SoLuong)) as decimal(18, 2))
			  END as decimal(18,2))

			-- ===== COMPUTED: so_luong_tra (từ #TraThuoc thay correlated subquery) =====
			, so_luong_tra = ISNULL(tt_tra.SoLuongTra, 0)

			-- ===== COMPUTED: sum_qty =====
			, sum_qty = sum(xncpct.SoLuong)

			-- ===== COMPUTED: qty_factor (hệ số nhân PT50/PT80/Ghep cho tính tiền) =====
			, qty_factor = CASE 
				WHEN clsyc.PT50 = 1 THEN 0.5
				WHEN clsyc.PT80 = 1 THEN 0.8
				WHEN clsyc.Ghep2 = 1 THEN 0.5
				WHEN clsyc.Ghep3 = 1 THEN 0.3
				ELSE 1.0 END

			-- ===== COMPUTED: don_gia_bv =====
			, Don_Gia_BV = isnull(xbn.DonGiaDoanhThu,xncpct.DonGiaHoTro)

			-- ===== COMPUTED: is_vtyt_goithau (flag VTYT gói thầu) =====
			, is_vtyt_tt04 = CASE WHEN PTVT.BenhAnPhauThuat_VTYT_Id IS NOT NULL And TT04.VATTU_TT04_ID IS NOT NULL THEN 1 ELSE 0 END
			, is_dg_dt_ge_ht = CASE WHEN ISNULL(xncpct.DonGiaDoanhThu,0) >= ISNULL(xncpct.DonGiaHoTro,0) THEN 1 ELSE 0 END
			...
			, Don_Gia = CASE WHEN PTVT.BenhAnPhauThuat_VTYT_Id IS NOT NULL 
							...
							case when ISNULL(PTVT.STT_Stent, 0) = 2  then CAST(TT04.DONGIA_TT04 as Decimal(18, 3)) * 1/2
							when ISNULL(PTVT.STT_Stent, 0) = 3 then CAST(TT04.DONGIA_TT04 as Decimal(18, 3)) * 1/3
							when ISNULL(PTVT.STT_Stent, 0) = 4 then CAST(TT04.DONGIA_TT04 as Decimal(18, 3)) * 1/3
							else CAST(TT04.DONGIA_TT04 as Decimal(18, 3))
							end 
						ELSE 
			...
			, tyle_tt = CASE WHEN clsyc.TyLeThanhToan IS NOT NULL
							THEN CASE WHEN map.TenField = '01' THEN clsyc.TyLeThanhToan*100
			...
			, tyle_thanhtoanBH = CAST( CASE WHEN ISNULL(dtl.TyLe,0) > 0 THEN dtl.TyLe 
							WHEN ISNULL(PTVT.STT_STENT,0) = 2 and d.MaHoatChat like N'%N06.02.020%' THEN 50
							WHEN ISNULL(PTVT.STT_STENT,0) = 3 and d.MaHoatChat like N'%N06.02.020%' THEN 33
							WHEN ISNULL(PTVT.STT_STENT,0) = 4 and d.MaHoatChat like N'%N06.02.020%' THEN 33
			...
			-- ===== COMPUTED: don_gia_thanhtien_bv (đơn giá dùng để tính Thanh_Tien) =====
			, don_gia_tt_bv = CASE WHEN PTVT.BenhAnPhauThuat_VTYT_Id IS NOT NULL 
								And TT04.VATTU_TT04_ID IS NOT NULL 
								And ISNULL(xncpct.DonGiaDoanhThu,0) >= ISNULL(xncpct.DonGiaHoTro,0) THEN CAST(xncpct.DonGiaDoanhThu as Decimal(18, 3))
							ELSE CAST(isnull(xbn.DonGiaDoanhThu,xncpct.DonGiaHoTro) as Decimal(18, 3)) END

			-- ===== COMPUTED: don_gia_thanhtien_bh (đơn giá dùng để tính Thanh_Tien_BH) =====
			, don_gia_tt_bh = CASE WHEN PTVT.BenhAnPhauThuat_VTYT_Id IS NOT NULL 
								And TT04.VATTU_TT04_ID IS NOT NULL 
								And ISNULL(xncpct.DonGiaDoanhThu,0) >= ISNULL(xncpct.DonGiaHoTro,0) THEN CAST(xncpct.DonGiaDoanhThu as Decimal(18, 3))
			...
			, T_TRANTT = CASE	
						WHEN PTVT.BenhAnPhauThuat_VTYT_Id IS NOT NULL And TT04.VATTU_TT04_ID IS NOT NULL And ISNULL(PTVT.STT_Stent, 0) = 2 THEN 18000000
						WHEN PTVT.BenhAnPhauThuat_VTYT_Id IS NOT NULL And TT04.VATTU_TT04_ID IS NOT NULL and d.MaHoatChat like N'%N06.02.020%' And ISNULL(PTVT.STT_Stent, 0) = 3 THEN 12000000
						WHEN PTVT.BenhAnPhauThuat_VTYT_Id IS NOT NULL And TT04.VATTU_TT04_ID IS NOT NULL and d.MaHoatChat like N'%N06.02.020%' And ISNULL(PTVT.STT_Stent, 0) = 4 THEN 12000000
						WHEN PTVT.BenhAnPhauThuat_VTYT_Id IS NOT NULL And TT04.VATTU_TT04_ID IS NOT NULL And ISNULL(PTVT.STT_Stent, 0) = 1 THEN CAST(TT04.DONGIA_TT04 as Decimal(18, 0))
						WHEN PTVT.BenhAnPhauThuat_VTYT_Id IS NOT NULL And TT04.VATTU_TT04_ID IS NOT NULL And ISNULL(PTVT.STT_Stent, 0) = 0 THEN CAST(TT04.DONGIA_TT04 as Decimal(18, 0))
					ELSE NULL END

			, MUC_HUONG_calc = case 
						when ISNULL(PTVT.STT_STENT,0) = 2 then 100
						when ISNULL(PTVT.STT_STENT,0) = 3 then 100
						when ISNULL(PTVT.STT_STENT,0) = 4 then 100
						else
			...
			-- ===== COMPUTED: muc_huong_pct (mức hưởng dạng phần trăm cho tính BHTT/BNCCT) =====
			, muc_huong_pct = CASE WHEN  ISNULL(@Tong_Chi,0) < 1 THEN 100 
							ELSE  
								CASE WHEN TT04.VATTU_TT04_ID IS NULL 
										THEN CASE WHEN xncpct.Muc_Huong <> dt.TyLe_2 THEN xncpct.Muc_Huong*100 
												ELSE case when dt.TyLe_2 is null then 0 else dt.TyLe_2 * 100 end  END
			...
			, muc_huong_pct_min = CASE WHEN  ISNULL(@Tong_Chi,0) < @MinBHYTChiTra THEN 100 
							ELSE  
								CASE WHEN TT04.VATTU_TT04_ID IS NULL 
										THEN CASE WHEN xncpct.Muc_Huong <> dt.TyLe_2 THEN xncpct.Muc_Huong*100 
												ELSE case when dt.TyLe_2 is null then 0 else dt.TyLe_2 * 100 end  END
			...
			, TT04_VATTU_TT04_ID = TT04.VATTU_TT04_ID
			, TT04_DONGIA_TT04 = TT04.DONGIA_TT04
			, xncpct.Muc_Huong
			, dt.TyLe_2
			, tn.TuyenKhamBenh_Id
			...
			, NGAY_THUCHIEN_YL = CASE WHEN ndv.MaNhomDichVu = '04' THEN REPLACE(CONVERT(varchar, COALESCE(kb.ThoiGianKham, yc.ThoiGianYeuCau), 112) + CONVERT(varchar(5), COALESCE(kb.ThoiGianKham, yc.ThoiGianYeuCau), 108), ':', '')
						...
						WHEN ndv.MaNhomDichVu IN ('0801','0802','0803') THEN NULL
			...
			, NGAY_KQ = case when li.PhanNhom = 'DV' and clsyc.YeuCauChiTiet_Id is not null  and map.TenField = '01' then 
				...
				replace(convert(varchar , COALESCE(kq.ngaykhoadulieu,kq.ThoiGianThucHien, bapt.ThoiGianKetThuc, baptth.ThoiGianKetThuc,yc.ThoiGianYeuCau), 112) + convert(varchar(5), COALESCE(kq.ngaykhoadulieu,kq.ThoiGianThucHien,bapt.ThoiGianKetThuc, baptth.ThoiGianKetThuc,yc.ThoiGianYeuCau), 108), ':','')
				end
			, ma_pttt=1	
			, NgoaiDinhXuat
			, MA_PTTT_QT = case when  li.PhanNhom = 'DV' and baptth.BenhAnPhauThuat_Id is not null then icd9.MaICD9_CM else null end
			, Nguoi_TH = isnull(case when li.PhanNhom = 'DV' and map.TenField = '01'  then nguoitaokb.SoChungChiHanhNghe
			...
			, li.PhanNhom
			, map.TenField
			, MA_DICH_VU_output = case when li.PhanNhom = 'DV' and map.TenField != '10' And map.TenField != '11' then case when tn.NgayTiepNhan > '20250731' then DV.MaQuiDinh else DV.MaQuiDinhCu end 
							   when LI.PhanNhom = 'DV' And map.TenField = '11' Then 'VC.' + bvct.TenBenhVien_Ru
							   else 
									case when PTVT.BenhAnPhauThuat_VTYT_Id IS NOT NULL And TT04.VATTU_TT04_ID IS NOT NULL then  dvktc.MaQuiDinh
									else null end
			...
			-- timestamp dùng cho t_bntt / t_bncct
			, ngay_th_datetime = CAST(SUBSTRING(CASE WHEN ndv.MaNhomDichVu = '04' THEN REPLACE(CONVERT(varchar, COALESCE(kb.ThoiGianKham, yc.ThoiGianYeuCau), 112) + CONVERT(varchar(5), COALESCE(kb.ThoiGianKham, yc.ThoiGianYeuCau), 108), ':', '')
								WHEN ndv.MaNhomDichVu IN ('0101', '0102', '0103', '0105', '0110', '0104', '0121', '0120') 
	...
	From	(
							...
							LEFT JOIN BenhAn ba (Nolock) ON ba.BenhAn_Id = @BenhAn_Id
	...
	LEFT JOIN	(
					...
					LEFT JOIN DM_DichVu dv (nolock) on dndv.DichVu_Id = dv.DichVu_Id
					LEFT JOIN DM_NhomDichVu ndv (nolock) on dv.NhomDichVu_Id = ndv.NhomDichVu_Id
					LEFT JOIN DM_LoaiDichVu ldv (nolock) on ndv.LoaiDichVu_Id = ldv.LoaiDichVu_Id
	...
	LEFT JOIN   dbo.DM_DichVu dv (Nolock)  on dv.DichVu_Id = clsyc.DichVu_Id
	 ...
	 left join (
		Select KhamBenh_Id, YeuCauChiTiet_Id, BacSiKham_Id, ThoiGianKham, ketthuckham, PhongBan_Id, NguoiTao_Id from KhamBenh (nolock) where TiepNhan_Id = @TiepNhan_ID
	...
	left join vw_NhanVien (nolock) bspt_vt on usmap.NhanVien_Id = bspt_vt.NhanVien_Id
	...
	left join DM_VATTU_TT04 TT04 (nolock) ON D.Duoc_Id = TT04.Duoc_ID
	...
	-- ===== JOIN #TraThuoc thay thế 12 correlated subquery =====
	LEFT JOIN #TraThuoc tt_tra on tt_tra.ToaThuoc_Id = xncpct.NoiTru_ToaThuoc_Id
	where 
...
group by xncpct.NoiTru_ToaThuoc_ID, CAST(kq.MoTa_Text AS NVARCHAR(MAX)), li.PhanNhom, case when tn.NgayTiepNhan > '20250731' then DV.MaQuiDinh else DV.MaQuiDinhCu end, dv.InputCode, d.MaDuoc, d.BHYT
		...
		, left (case when map.TenField in ('12','02') then  isnull(gb.MoTa ,case 
				...
				, isnull(case when li.PhanNhom = 'DV' and map.TenField = '01'  then nguoitaokb.SoChungChiHanhNghe
									when li.PhanNhom = 'DV' and ptyc.BenhAnPhauThuat_YeuCau_Id is not null then dbo.Get_MaBacSi_XML3_By_BenhAnPhauThuat_Id(BAPTTH.BenhAnPhauThuat_Id)
									when li.PhanNhom = 'DV' and kq.CLSKetQua_Id is not null then bskq.SoChungChiHanhNghe
									else NULL end
					,
				...
				-- thêm GROUP BY cho computed columns mới
				, ISNULL(tt_tra.SoLuongTra, 0)
)
-- ========== SELECT cuối cùng: Tính Thanh_Tien, T_BHTT, T_BNTT, T_BNCCT từ computed columns ==========
SELECT [ID] = row_number () OVER (ORDER BY (SELECT 1))
	  ,[MA_LK] = c.Ma_Lk
      ,[STT] = row_number () OVER (ORDER BY (SELECT 1))
      ,[MA_DICH_VU] = c.ma_dich_vu
      ,[MA_PTTT_QT] = c.MA_PTTT_QT
      ,[MA_VAT_TU] = c.ma_vat_tu
      ,[MA_NHOM] = c.MA_NHOM
      ,[GOI_VTYT] = c.GOI_VTYT
      ,[TEN_VAT_TU] = c.TEN_VAT_TU
      ,[TEN_DICH_VU] = c.ten_dich_vu
      ,[MA_XANG_DAU] = NULL
      ,[DON_VI_TINH] = c.don_vi_tinh
      ,[PHAM_VI] = c.PHAM_VI
      ,[SO_LUONG] = CAST(c.adj_qty - c.so_luong_tra as decimal(18,2))
      ,[DON_GIA_BV] = c.Don_Gia_BV
      ,[DON_GIA_BH] = c.Don_Gia
      ,[TT_THAU] = c.TT_THAU
      ,[TYLE_TT_DV] = c.tyle_tt
      ,[TYLE_TT_BH] = case when isnull(c.T_TRANTT,0) <> 0 then 100 else c.tyle_thanhtoanBH end
      ,[THANH_TIEN_BV] = CAST(CAST(c.sum_qty * c.qty_factor * c.don_gia_tt_bv as decimal(18,2)) - CAST(c.don_gia_tt_bv * c.so_luong_tra as Decimal(18, 2)) as decimal(18,2))
      ,[THANH_TIEN_BH] = case when isnull(c.T_TRANTT,0) <> 0 then c.Don_Gia * (CAST(c.adj_qty - c.so_luong_tra as decimal(18,2)))
                          else CAST(CAST(c.sum_qty * c.qty_factor * c.don_gia_tt_bh as decimal(18,2)) - CAST(c.don_gia_tt_bh * c.so_luong_tra as Decimal(18, 2)) as decimal(18,2)) * (c.tyle_thanhtoanBH/100)
                          end
      ,[T_TRANTT] = c.T_TRANTT
      ,[MUC_HUONG] = c.MUC_HUONG_calc
      ,[T_NGUONKHAC_NSNN] = 0
      ,[T_NGUONKHAC_VTNN] = 0
      ,[T_NGUONKHAC_VTTN] = 0
      ,[T_NGUONKHAC_CL] = 0
      ,[T_NGUONKHAC] = 0
	  -- T_BNTT = Thanh_Tien_BH - T_BHTT  (computed từ các giá trị trung gian)
	  ,[T_BNTT] = CAST(CAST(c.sum_qty * c.qty_factor * c.don_gia_tt_bv as decimal(18,2)) - CAST(c.don_gia_tt_bv * c.so_luong_tra as Decimal(18, 2)) as decimal(18,2))
				  - case when isnull(c.T_TRANTT,0) <> 0 then c.Don_Gia * (CAST(c.adj_qty - c.so_luong_tra as decimal(18,2)))
                    else CAST(CAST(c.sum_qty * c.qty_factor * c.don_gia_tt_bh as decimal(18,2)) - CAST(c.don_gia_tt_bh * c.so_luong_tra as Decimal(18, 2)) as decimal(18,2)) * (c.tyle_thanhtoanBH/100)
                    end
      ,[T_BNCCT] = case when c.TuyenKhamBenh_Id = 1157 And c.ngay_th_datetime < CAST('20210101' as smalldatetime)
						THEN 
							CASE WHEN ISNULL(@Tong_Chi,0) < 1 THEN 0
							ELSE
								CAST((CAST(c.sum_qty * c.qty_factor * c.don_gia_tt_bh as decimal(18,2)) - CAST(c.don_gia_tt_bh * c.so_luong_tra as Decimal(18, 2)))*0.4 as decimal(18,2))
								- CASE WHEN c.TT04_VATTU_TT04_ID IS NULL THEN
									CAST(CAST(CAST(c.DonGiaHoTro * c.sum_qty * c.qty_factor as Decimal(18, 2)) - CAST(c.DonGiaHoTro * c.so_luong_tra as Decimal(18, 2)) as decimal(18,2))
									* case when c.TuyenKhamBenh_Id = 1157 then c.Muc_Huong*100 else c.muc_huong_pct_min end /100 as decimal(18,2))
								ELSE
									CAST(CAST(CAST(c.DonGiaHoTroChiTra * c.sum_qty * c.qty_factor as Decimal(18, 2)) - CAST(c.DonGiaHoTroChiTra * c.so_luong_tra as Decimal(18, 2)) as decimal(18,2))
									as decimal(18,2))
								END
							END
					else
						CAST(CAST(c.DonGiaHoTro * c.sum_qty * c.qty_factor as Decimal(18, 2)) - CAST(c.DonGiaHoTro * c.so_luong_tra as Decimal(18, 2)) as decimal(18,2))
						- CASE WHEN c.TT04_VATTU_TT04_ID IS NULL THEN
							CAST(CAST(CAST(c.DonGiaHoTro * c.sum_qty * c.qty_factor as Decimal(18, 2)) - CAST(c.DonGiaHoTro * c.so_luong_tra as Decimal(18, 2)) as decimal(18,2))
							* c.muc_huong_pct_min /100 as decimal(18,2))
						ELSE
							CAST(CAST(CAST(c.DonGiaHoTroChiTra * c.sum_qty * c.qty_factor as Decimal(18, 2)) - CAST(c.DonGiaHoTroChiTra * c.so_luong_tra as Decimal(18, 2)) as decimal(18,2))
							as decimal(18,2))
						END
					end
      ,[T_BHTT] = CASE WHEN c.TT04_VATTU_TT04_ID IS NULL THEN
					CAST(CAST(CAST(c.DonGiaHoTro * c.sum_qty * c.qty_factor as Decimal(18, 2)) - CAST(c.DonGiaHoTro * c.so_luong_tra as Decimal(18, 2)) as decimal(18,2))
					* c.muc_huong_pct /100 as decimal(18,2))
				ELSE
					CAST(CAST(CAST(c.DonGiaHoTroChiTra * c.sum_qty * c.qty_factor as Decimal(18, 2)) - CAST(c.DonGiaHoTroChiTra * c.so_luong_tra as Decimal(18, 2)) as decimal(18,2))
					as decimal(18,2))
				END
      ,[MA_KHOA] = c.[MA_KHOA]
      ,[MA_GIUONG] = c.[MA_GIUONG]
      ,[MA_BAC_SI] = c.[MA_BAC_SI]
      ,[NGUOI_THUC_HIEN] = c.Nguoi_TH
      ,[MA_BENH] = c.[MA_BENH]
      ,[MA_BENH_YHCT] = case when c.ten_dich_vu = N'Khám YHCT' then @ICD_CHINH_YHCT + ';' + @ICD_PHU_YHCT else '' end
      ,[NGAY_YL] = c.[NGAY_YL]
      ,[NGAY_TH_YL] = c.[NGAY_THUCHIEN_YL]
      ,[NGAY_KQ] = c.[NGAY_KQ]
      ,[MA_PTTT] = c.[ma_pttt]
      ,[VET_THUONG_TP] = NULL
      ,[PP_VO_CAM] = c.PPVC
	  ...
	  ,[LoaiBenhPham_Id] = 
			CASE 
				WHEN c.MA_DICH_VU_output IN (N'24.0017.1714',N'24.0001.1714', N'24.0005.1716', N'24.0003.1715') THEN c.LoaiBenhPham_Id
				ELSE NULL
			END
	  ,[ChucDanh_id] = c.chuc_danh
	  ,[MoTa_Text] = c.MoTa_Text
	  ,[KET_LUAN] = c.KetLuan
	  ,[TrinhTuThucHien] = CAST(bapt.TrinhTuThucHien AS NVARCHAR(MAX))
	  ,[PhuTang] = c.PhuTang
FROM CTE_Base c
LEFT JOIN BenhAnPhauThuat bapt (nolock) ON bapt.BenhAnPhauThuat_Id = ISNULL(c.ba1, c.ba2)
WHERE CAST(c.adj_qty - c.so_luong_tra as decimal(18,2)) > 0

-- ========== PHẦN 10: Dọn dẹp temp table ==========
DROP TABLE IF EXISTS #TraThuoc
